...
- **永久性错误可重试** — 透明通道和路径冲突导致的错误不再显示重试按钮
- **官方 API 无法压缩** — shrink 响应的 output URL 在 Location 响应头，convert 返回图片二进制
